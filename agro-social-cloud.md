...
> **Bez interaktivního checkpointu.** Na rozdíl od `agro-social` tahle verze **nečeká na potvrzení** —
...
## Sdílené proměnné (per článek, viz mapování kategorií a region v `agro-social`)

Stejné jako v `agro-social`: `[TITULEK]`, `[KATEGORIE]`, `[DATUM]`, `[DATUM_SLUG]`, `[COVER_URL]`, `[ID]`,
`[CLANEK_URL]`. Mapování `category_id → [KATEGORIE]` a auto-detekce regionu zůstávají beze změny
(viz `agro-social/SKILL.md`).
...
- Ulož vybrané jako `$article1`, `$article2`, `$article3` (JSON objekty) — z každého vytáhni
  ...
  stejně jako v `agro-social` Kroku 1.

---

## Krok 2 — Pro KAŽDÝ ze 3 článků: naplň Canva šablonu (MCP)

Identické s `agro-social` Krokem 2 (šablona `DAHOBdpJ1tk` — ProfiFarmář 4:5 Stacked, `copy-design` →
...
Identické s `agro-social` Krokem 3 — `mcp__canva__export-design`, **pouze `width: 1080`**, nikdy
...
Uprav podle skutečného `public_id` schématu, které používáš v `agro-social` (folder `ČLÁNKY`).

---

## Krok 5 — Copywriting (1 IG + 1 FB text per článek, 3×)

Stejná stylistická pravidla jako v `agro-social` Kroku 5 (IG: 1–3 věty, max 150 znaků, přesně 5
...
**Klíčový rozdíl oproti `agro-social`:** primární `mode` je tady **vždy `customScheduled`**
...
git commit -m "agro-social-cloud: posted $(date -u +%Y-%m-%d)"
git push
```
...
## Troubleshooting (doplňky k `agro-social`)
...
| Canva element nenalezen | Element IDs vždy z aktuální `start-editing-transaction` response, ne z tabulky v `agro-social` |
